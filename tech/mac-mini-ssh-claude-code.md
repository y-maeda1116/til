...
Windows PCからSSHでMac miniに接続し、Claude Codeを複数リポジトリで並行運用する場合の推奨構成をまとめます。Windows Updateによる再起動でSSHが切断されてセッションが失われるのを防ぐため、Mac側でtmuxを常時稼働させます。
...
- [3. セットアップスクリプト一覧](#3-セットアップスクリプト一覧)
...
### 3.2 ~/bin/work.sh（シンプル版 - 推奨）

以下の内容を `~/bin/work.sh` に保存してください。
...
SSH接続後、`work` の1コマンドで完全復元できます。

### 3.3 ~/.tmuxinator/work.yml（YAML版 - 高度な設定向け）
...
以下の内容を `~/.tmuxinator/work.yml` に保存してください。
...
特定リポジトリでsecurityの参照頻度が高い場合、そのwindow内をpane分割して同時表示可能。
...
以下の行を `/etc/ssh/sshd_config` に追加または編集してください。
...
```bash
sudo apt install mosh  # Ubuntu/Debian系
# または
brew install mosh  # WSL with Homebrew
```

**その他:**
- MobaXtermなどのSSHクライアントを利用
...
- **グループポリシー(Pro以上)**: `gpedit.msc` で「コンピュータの構成 → 管理用テンプレート → Windows コンポーネント → Windows Update」→「自動更新を構成する」で「ダウンロードと自動インストールを通知」に変更。
...
2. Mac miniに接続。
3. 以下の `work.code-workspace` ファイルを作成。
...
ここまでの構成でWindows Update問題は実用上ほぼ解消できますが、**さらに根本的に切り離したい**場合は「Mac miniをリモートデスクトップで操作する」という選択肢があります。
...
| **Jump Desktop** | Fluid (独自) / RDP / VNC | Mac向け定番。Fluidプロトコルが高速・低遅延。ファイル転送・マルチモニタ対応 | 速い | 有料(買い切り or サブスク) | 業務利用・こだわり派 |
| **Parsec** | 独自(低遅延ストリーミング) | 元はゲーム向け。フレームレート・遅延ともに最良クラス。MacはホストにできるがApple Siliconは制約あり | 最速 | 個人無料 / 商用有料 | ゲーム感覚で操作したい派 |
| **AnyDesk** | 独自(DeskRT) | 軽量、クロスプラットフォーム、業務利用実績多数。社内ポリシーで承認されているケースが多い | 速い | 個人無料 / 商用有料 | 業務利用・IT部門承認向け |
...
| **NoMachine** | NXプロトコル | 高速。Macクライアントは安定。設定はやや上級者向け | 速い | 個人無料 / 商用有料 | 技術者向け |
...
- [ ] `brew install tmux` で tmux をインストール
- [ ] `~/.tmux.conf` を作成(mouse on / history-limit / base-index)
- [ ] `~/bin/work.sh` を作成して `chmod +x ~/bin/work.sh` で実行可能に
- [ ] Mac側 `/etc/ssh/sshd_config` に ClientAlive設定を追加
- [ ] Windows側 `~/.ssh/config` に ServerAliveInterval設定を追加
- [ ] securityリポジトリの参照方式を決定(symlink / 親dir claude / pane分割)
- [ ] Windows Updateのアクティブ時間・再起動オプション見直し
- [ ] (任意) `brew install mosh` で mosh をインストール
- [ ] (任意)VS Code Remote-SSH + Multi-root Workspace設定
...
本ドキュメントは社内・個人での自由な再配布・改変を想定しています。
